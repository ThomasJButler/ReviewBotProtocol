...
- A finding whose quoted evidence lands on a line the pull request never touched is dropped, from either model, unless the quote is a line the change removed, which the prompt asks to be reported on the line that now lacks it. `CONTEXT_LINE_FINDINGS` (`drop`) also offers `keep`, `downgrade` one severity step and `confidence`, the last keeping such a finding only at or above `CONTEXT_LINE_MIN_CONFIDENCE` (0.9). Eight replayed runs show recall, false positives per clean diff, every injection report and the score unchanged under all four policies (`docs/benchmarks/2026-09-06-round-three.md`), so the count decided it: `drop` removes 4 of the 119 findings the bot has posted on this repository, one of them among the 22 two independent judges found worthless.
