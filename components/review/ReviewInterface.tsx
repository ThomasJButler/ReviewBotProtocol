...
export default function ReviewInterface({
  ...
  // Mock review results generator
  const generateMockResults = useCallback(
    (request: ReviewRequest): ReviewResultsDisplay => {
      const mockFindings: ReviewFinding[] = [
        {
          id: '1',
          title: 'SQL Injection Vulnerability',
          description:
            'User input is directly concatenated into SQL query without proper sanitization. This could allow attackers to execute arbitrary SQL commands.',
          severity: 'critical',
          category: 'security',
          lineNumber: 42,
          fileName: request.files?.[0]?.name || 'input.js',
          code: 'const query = `SELECT * FROM users WHERE id = ${userId}`;',
          suggestion:
            'Use parameterized queries or prepared statements to prevent SQL injection.',
          impact:
            'Attackers could potentially access, modify, or delete sensitive data from the database.',
          effort: 'medium',
          references: [
        ...
        },
        {
          ...
          description:
            'The query uses SELECT * which retrieves all columns, potentially loading unnecessary data and impacting performance.',
          severity: 'medium',
          ...
          impact:
            'Increased memory usage and slower query execution, especially with large datasets.',
          effort: 'low',
        },
        {
          id: '3',
          ...
          description:
            'Async operation is not wrapped in try-catch block, which could lead to unhandled promise rejections.',
          severity: 'high',
          category: 'quality',
          lineNumber: 28,
          fileName: request.files?.[0]?.name || 'input.js',
          ...
          suggestion:
            'Wrap async operations in try-catch blocks or use .catch() method.',
          impact:
            'Application may crash or behave unexpectedly when errors occur.',
          effort: 'low',
        },
        {
          id: '4',
          ...
          description:
            'API key is hardcoded in the source code, which is a security risk.',
          severity: 'high',
          category: 'security',
          lineNumber: 8,
          fileName: request.files?.[0]?.name || 'input.js',
          ...
          impact:
            'API keys could be exposed if code is shared or stored in version control.',
          effort: 'low',
        },
      ]
      ...
      const mediumCount = mockFindings.filter(
        f => f.severity === 'medium'
      ).length
      const lowCount = mockFindings.filter(f => f.severity === 'low').length

      const securityCount = mockFindings.filter(
        f => f.category === 'security'
      ).length
      const performanceCount = mockFindings.filter(
        f => f.category === 'performance'
      ).length
      const qualityCount = mockFindings.filter(
      ...
      return {
        id: `review-${Date.now()}`,
        timestamp: new Date().toISOString(),
        summary: `Analysis complete! Found ${mockFindings.length} issues across security, performance, and code quality. ${criticalCount > 0 ? 'Critical security vulnerabilities require immediate attention.' : 'No critical issues detected.'} Overall code quality score: ${overallScore}/100.`,
        overallScore,
        ...
        metrics: {
          ...
          performanceIssues: performanceCount,
          qualityIssues: qualityCount,
          linesAnalyzed: request.content?.split('\n').length || 150,
          filesAnalyzed: request.files?.length || 1,
        },
