...
1. GitHub sends a `pull_request` webhook (opened, synchronize, reopened, ready_for_review). The body is capped, the HMAC-SHA256 signature is verified over the raw bytes, the delivery is recorded by its id and by the hash of its signed body (a repeat of either is ignored, and the hash record outlives the review data), and the request returns 202. Nothing slow happens in the request.
...
3. It fetches the PR and its changed files with a GitHub App installation token. Deleted, binary, generated, vendored, non-code and oversized files are skipped (markdown counts as non-code while `REVIEW_MARKDOWN` is off, which is the default), and so is anything secret-bearing by name (`.env`, `*.pem`, `id_rsa`, `*.tfvars` and so on).
...
Leaves the machine (local mode; in hosted mode read "the server"): requests to `api.github.com` only (fetch the PR and its files, post the review, and with `FILE_CONTEXT=true` read each reviewed file at the head commit, one per reviewed file the pull request did not add). Nothing is sent to any model provider, error tracker or tracing service, and the process refuses to start if the environment says otherwise (`STRICT_LOCAL`, on by default, fails on `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `LANGCHAIN_API_KEY`, `LANGSMITH_API_KEY`, `SENTRY_DSN` and on any LangSmith tracing variable set to true).
...
Measured again on 2026-09-06 (docs/benchmarks/2026-09-06-round-three.md), two repeats, after the ground truth was widened on one case and the judge's rules tightened: the shipped prompt reaches recall 0.98 with 0.25 false positives per clean diff (0.16 at medium or worse), reports the hostile text in 16 of 32 rows, obeys nothing, and scores 0.918; a candidate written against its failure rows lost (0.896) and did not ship. The verifier text was replaced by a candidate that keeps five injection reports the old one refuted (recall 0.96 against 0.933 on the same rows), still off by default. The cross-examiner's schema was measured with its note first and with its verdicts first, like for like on the full corpus; the default stays verdicts first, and with the bare-tag rule the reading found, the pair under it scores 0.929 (recall 0.953, 0.09 false positives per clean diff, every report kept, 182 of 182 rows examined), above the reviewer alone at 0.918 on the same rows. The corpus measures recall on planted diffs; `scripts/precision.py` measures the other half, precision on a fixed set of eleven real pull requests of this repository, with every finding judged by two independent readers against the code and the judged set stored beside the corpus in `tests/precision/`, so a round only pays for the findings that are new. `replay` re-scores a recorded round through a changed pipeline with no model loaded, which is how a post-filter change is judged on real code rather than on planted diffs; the protocol and the rounds are docs/benchmarks/PRECISION.md.

## Limits

Every setting, with its type, default and what raising it costs, is in `docs/SETTINGS.md`, generated from the settings class by `scripts/settings_reference.py` (a test keeps the two equal).

`MAX_FILES_PER_REVIEW` (25), `MAX_PATCH_BYTES` (32 000, which must fit `OLLAMA_NUM_CTX`), `MAX_WEBHOOK_BODY_BYTES` (2 MiB), `MAX_INLINE_COMMENTS` (25), `REVIEW_TIMEOUT_SECONDS` (3600: a rewrite-sized diff costs about two minutes a file, twice with the cross-examiner; a review that hits the ceiling posts the files that finished, lists the rest under Not reviewed, and records how far it got), `REVIEW_SECONDS_PER_FILE` (300: a review's own budget is its file count times this, capped by the ceiling, so a small pull request stops holding the queue for an hour; 0 leaves only the ceiling), `MIN_FINDING_CONFIDENCE` (0.5). `FILE_CONTEXT` (false) sends the file the diff came from beside the diff, whole when it fits and otherwise the parts of it within `FILE_CONTEXT_LINES` (60) of each hunk, merged where they overlap: the two together get `OLLAMA_NUM_CTX` less `OLLAMA_NUM_PREDICT` less the prompt's own overhead at about 2.8 bytes a token, which is 36,355 bytes at 16384 and 82,230 at 32768, so a patch that fills the budget gets no context rather than being skipped, and the size gate still reads the patch alone. It costs one extra `api.github.com` call per reviewed file the pull request did not add (an added file's diff is already the whole file) and the App installation's Contents read permission, which an existing installation must accept in GitHub's UI before the switch is turned on; without it the token mint narrows and retries, so reviews keep running on the patch alone, prompt included, which is the same thing as the switch being off. Roughly double the prompt tokens means more seconds a file, so `REVIEW_SECONDS_PER_FILE` may need raising with it on. It is off until a measured round says otherwise. A review that fails before anything was posted leaves one line on the pull request naming the exception; the reason stays on the dashboard. Draft PRs are skipped unless `REVIEW_DRAFTS=true`, and pull requests opened by a bot (dependabot, renovate) unless `REVIEW_BOT_PULL_REQUESTS=true`. A skipped delivery is still recorded, as `skipped_draft` or `skipped_bot`, so the Status page shows what GitHub sent. `REVIEW_RETENTION_DAYS` (365): reviews and their findings older than this are deleted at startup and then nightly, never a row still running, and a webhook delivery that old is stripped to a tombstone of its id, the SHA-256 of its signed body and the time it arrived, shown as Expired on the Status page, because that hash is the only thing that keeps a captured delivery from being replayed; 0 keeps everything. A deleted review's dashboard link becomes a 404, so export what is worth keeping first: `.venv/bin/python scripts/export_reviews.py --out reviews.json --older-than 365` writes every column of every selected review, findings included, as JSON. Reference machine: a 2021 M1 Max with 32 GB runs `qwen3.5:9b` at about 13 seconds per file and, with `gemma4:12b` cross-examining, about 34 seconds per file, one model resident at a time.
