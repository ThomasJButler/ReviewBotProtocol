...
FOOTER_CROSS = ("Generated by ReviewBot Protocol: reviewed by `{model}`, cross-examined by `{cross_model}`, via Ollama. "
                "Findings are model output and may be wrong; verify before acting.")
FOOTER = ("Generated by ReviewBot Protocol using `{model}` via Ollama. "
          "Findings are model output and may be wrong; verify before acting.")

_HTML_TAG = re.compile(r"<[^>]*>", re.S)
# A reference definition may put its destination on the next line (CommonMark allows one
...
def _github_url(url: str) -> bool:
    url = (url or "").strip()
    if url.startswith("//"):
        url = "https:" + url  # scheme-relative: a browser would resolve it to https on GitHub
    try:
        parts = urlparse(url)
    except ValueError:
        return False
...
def sanitise(text: str, limit: int, code: bool = False, inline: bool = False) -> str:
    ...
    if code:
        ...
        def _keep(m: "re.Match[str]") -> str:
            if _github_url(m.group(2)):
                kept_links.append(f"[{m.group(1)}]({m.group(2).strip()})")
                return f"\x00{len(kept_links) - 1}\x00"
            return m.group(1)

        out = _MD_LINK.sub(_keep, out)
        out = _BARE_URL.sub(lambda m: m.group(0) if _github_url(m.group(0) if "://" in m.group(0) else "https://" + m.group(0)) else "[link removed]", out)
        out = _SCHEME_RELATIVE.sub(lambda m: m.group(0) if _github_url(m.group(0)) else "[link removed]", out)
        out = _LEFTOVER_LINK.sub("] (", out)  # whatever link syntax survived cannot open a link now
...
def _provenance_note(f, cross_model: Optional[str]) -> str:
    """One line saying which model raised a finding or doubted it, for the
    posted comment and the dashboard. Empty when there is nothing to say."""
    if not cross_model:
        return ""
    if getattr(f, "source_model", "") == cross_model:
        return f"Found by the cross-examiner (`{sanitise(cross_model, 100, code=True)}`)."
    if getattr(f, "cross_verdict", None) == "false_positive":
        return "The cross-examiner disagreed: " + sanitise(getattr(f, "cross_reason", "") or "no reason given", 200, inline=True)
    return ""


def _inline_body(sev: Severity, category: str, title: str, recommendation: str, evidence: str, note: str = "") -> str:
...
def render_review(results: Sequence[FileReviewResult], *, model: str, skipped: Sequence[Tuple[str, str]] = (),
        ...
        if len(comments) < max_inline:
            comments.append({"path": path, "line": f.line, "side": "RIGHT",
                             "body": _inline_body(f.severity, f.category.value, f.title, f.recommendation, f.evidence,
                                                  _provenance_note(f, cross_model))})
        else:
    ...
    if cross_model:
        lines.append(f"Cross-examined by `{sanitise(cross_model, 100, code=True)}`: {cross_added} added, {cross_refuted} refuted.")
    if is_fork:
    ...
    footer = "---\n" + (FOOTER_CROSS.format(model=sanitise(model, 100, code=True), cross_model=sanitise(cross_model, 100, code=True))
                        if cross_model else FOOTER.format(model=sanitise(model, 100, code=True)))
    body = "\n".join(lines)
