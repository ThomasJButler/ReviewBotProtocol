...
Six verifiers (one per finding, each reading the current branch rather than the scan's notes) re-checked the scan's findings against the code as it stood at 2df1c5f, after three days of further change. F1, F2, F16 and F17 fixed, with pins in `backend/tests/test_scan_round.py`; F14 and F15 partly: the replay key is the signed body's hash, but a captured body whose delivery ended failed or interrupted is accepted again under a fresh id (that is also what lets an operator redeliver after a crash), and the queue let such an older head cancel an in-flight review of a newer one. Closed the same day: the payload's `pull_request.updated_at` travels with the job and an older delivery is answered "stale" without touching the newer job (`services/review_queue.py`, pinned in `tests/test_queue.py`); the widened link pattern's whitespace runs are bounded so a bracket followed by a flood of spaces is linear (F17's one residual); the hosted-mode doc's example tunnel command carries the pinned host-key options (F1's one residual, the script already did); and a test reads `gpu-up.sh` for the pin, since nothing else exercises the script. Two residuals stay as written down: the regex work under `asyncio.to_thread` does not leave the event loop under the GIL, which the linear bounds make harmless; and the replay check's read-then-insert has no unique index behind it, which the queue's own duplicate check makes harmless (one review runs, two delivery rows may be recorded). From 2026-09-12 the F14 and F15 seam closes at the retention cutoff: a delivery that old is stripped to a tombstone whose status is neither failed nor interrupted, so a body older than `REVIEW_RETENTION_DAYS` is refused as a replay however it ended; see "Claude Security scan (2026-09-08)".

## Claude Security scan (2026-09-08)

Tom ran the Claude Security plugin again (`/claude-security`, the v1.2 range, high effort) on 2026-09-08. The report lives in `CLAUDE-SECURITY-20260908-004806/`, kept out of commits by its own `.gitignore`. One caveat on its coverage: the inventory declared the retention sweep, the cancellation path, the export script and the frontend out of scope on a false reading of the diff, so an empty result there means unexamined, not clean. Two findings survived its panel:

- F1 [low] backend/database/repositories/webhook_repository.py:71: the nightly sweep deleted the delivery rows the replay check reads, while GitHub's HMAC carries no timestamp, so after `REVIEW_RETENTION_DAYS` a captured signed body authenticated again, and the completed-review backstop had gone with it. Fixed on 2026-09-12: the sweep strips a delivery that old to a tombstone of its id, the SHA-256 of its signed body, its event and its received time, with status `expired` and the repository, pull request, head, action and review link cleared, and `record()` already answers any status outside failed and interrupted as a duplicate; pinned by a test in `backend/tests/test_retention.py` that replays a captured body after the sweep and expects `duplicate`. The alternative the report offered, refusing a payload older than a freshness window, was considered and rejected: `pull_request.updated_at` is optional in the payload, ping and installation events carry no such field yet are recorded and replayable, and GitHub's own redelivery arrives legitimately late.
- F2 [low] backend/services/ai_reviewer.py:140: polynomial backtracking in the praise pattern on model text derived from the diff, 2.8 s for one 600-character recommendation and 84 s for thirty. Already fixed in 1.2.0 by making each subject word whitespace-free, pinned by a timing test in `backend/tests/test_review_generation.py`.
...
- 2026-09-12: F1 of the 2026-09-08 scan fixed: a delivery past retention becomes a tombstone instead of being deleted, so a captured body stays a duplicate for as long as the database exists; the F14 and F15 redelivery seam now closes at the cutoff.
