...
- **Skip pull requests opened by bots.** Done in v1.2: `REVIEW_BOT_PULL_REQUESTS` (off) decides whether a pull request whose author is a bot is reviewed; the delivery is recorded as `skipped_bot` either way. Dependabot opened nine in one afternoon; each would have queued an hour of the laptop.
