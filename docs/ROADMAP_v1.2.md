...
- **From the two reviews of PR 13.** A `global-error.tsx` so a render error in the root layout shows a fallback (none has ever existed on any branch; the note that main had one was wrong); a unique index on the delivery body hash so the replay check is a constraint rather than a read-then-insert (done in v1.2, added to existing databases at startup); `CardTitle` and `CardDescription` render as `div`, which is shadcn's default and a fair accessibility nit for a heading; one unused import (`func` in `database/models.py`).
