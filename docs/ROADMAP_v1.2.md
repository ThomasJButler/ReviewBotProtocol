...
- PR 13 (`v1.1-Local-AI` into `main`) merged at 18:17 UTC as `5121aa3`, after three reviews by the bot and five internal readers.
...
- **The forged-delimiter token.** A forged data-block marker inside the diff is rewritten to `[data-marker]`, and the verifier read that as the pipeline's own framing and stood down. Rewrite it to `[forged-data-marker]` (a pipeline change, no measurement needed) and name it in both prompts (a prompt change, so a round-four candidate measured before it ships). The verifier's one remaining silenced case.
- **Praise filed as a finding.** "aria-live for status" with a recommendation of "correctly implements" passes every postprocess rule. A rule for a recommendation that says the code is right, or a prompt sentence, or both.
