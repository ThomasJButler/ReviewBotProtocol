...
- **Praise filed as a finding.** Done in v1.2 as a pipeline rule, measured by replay (0.918 to 0.929 for the reviewer alone, one clean-diff false positive fewer, recall unchanged; `docs/benchmarks/2026-09-06-round-three.md`, the evening addendum). The prompt sentence is still a round-four candidate.
- **From the two reviews of PR 13.** A `global-error.tsx` so a render error in the root layout shows a fallback (done in v1.2; none had ever existed on any branch, the note that main had one was wrong); a unique index on the delivery body hash so the replay check is a constraint rather than a read-then-insert (done in v1.2, added to existing databases at startup); `CardTitle` renders as `div`, shadcn's default, so the section cards now pass `as` with a heading (done in v1.2); the unused `func` import in `database/models.py` is gone.
- **Skip pull requests opened by bots.** Done in v1.2: `REVIEW_BOT_PULL_REQUESTS` (off) decides whether a pull request whose author is a bot is reviewed; when it is not, the delivery is recorded as `skipped_bot`, so the Status page still shows what GitHub sent. Dependabot opened nine in one afternoon; each would have queued an hour of the laptop.
...
- **A settings reference, generated.** Every setting in `backend/config/settings.py` with its meaning, default, unit, what changes when you raise it and what it costs (`REVIEW_TIMEOUT_SECONDS` against files and diff size; `MAX_PATCH_BYTES` against `OLLAMA_NUM_CTX`; `CROSS_EXAMINE_SEQUENTIAL` against memory; `STRICT_LOCAL` and what it refuses). Generated in v1.2: `docs/SETTINGS.md` from the docstring under every field (`scripts/settings_reference.py`, with a test that keeps the file and the class equal); the site renders that file.
...
- **Troubleshooting from real runs.** The table in `docs/TEST_PLAN.md` (which since v1.2 carries the rows below) plus whatever the site's own readers turn up: a pasted URL in `ALLOWED_HOSTS`, the webhook URL without its path, a review that hits the ceiling and posts nothing, the placeholder rule leaving an "example" key in the clear, Ollama relaunching itself after a kill.
...
- Done in v1.2: `docs/HOSTED_MODEL_PLAN.md` and `docs/SECURITY_REVIEW.md` open with a dated note saying what they are, what still holds and what superseded the rest.
- The chain scripts under `~/ReviewBot-runs/` are the measurement's provenance and live outside the repository by design; `docs/benchmarks/HOW_TO_RUN_A_ROUND.md` (done in v1.2) has the chain's shape: pre-flight, pick with fallback, one model at a time, Ollama off at the end, the laptop on mains, and what went wrong before.
