...
- One narrow header: wordmark, three nav links (Reviews, Status, Setup; the detail page is reached from the table), theme toggle. No sticky glass, no gradients, no glow.
...
- Motion: none beyond focus and hover transitions and the shadcn menu open animation; prefers-reduced-motion respected.
...
app/api/**, app/demo, app/pull-requests, app/review, app/history (replaced), components/review/_, components/layout/Header and Footer, components/charts, contexts/AuthContext, hooks/_, lib/utils calculateComplexity, scripts/generate-_.js, vercel.json, the Google Fonts import, the matrix theme.
Packages dropped: @monaco-editor/react, recharts, react-dropzone, next-auth, @auth/prisma-adapter, @langchain/_, langchain, @octokit/*, axios, zustand, react-hook-form, @hookform/resolvers, react-hot-toast, date-fns, zod (unless a form needs it), puppeteer, sharp, @tailwindcss/forms, @tailwindcss/typography (a small prose style is written by hand).
...
All four screens as described, verified in a browser against a seeded local backend and against a dead backend (every page renders an operator-facing alert rather than a 500). Lighthouse on the reviews and status pages: accessibility 100, best practices 100. The dashboard binds to 127.0.0.1, refuses any other Host header in middleware, sends a Content-Security-Policy and has 15 unit tests (vitest) for the link allowlist, date handling and the API client's guards. The production build's HTML references no external host, font or script.
...
- The Posted tab shows the body only; there is no count of inline comments (the backend does not store it) and the GitHub link lives in the page header.
...
- Versions: next 15.5, react 19, tailwindcss 4 with `@tailwindcss/postcss`, the unified `radix-ui` package the shadcn CLI now installs, eslint 9 with a flat config (9.x reached end of life on 2026-08-06; the move to 10 waits on eslint-config-next). The shadcn Tailwind layer is vendored into `app/shadcn-tailwind.css`, so the shadcn CLI is not a dependency at all.
