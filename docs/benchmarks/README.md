...
| [2026-09-12](2026-09-12-file-context.md)                                                                         | The file beside the hunk, tier one only and a stub: the off-path replay proving `FILE_CONTEXT=false` is the incumbent row for row, and the budget table. No model has seen a file listing yet; the two measured rounds say "not yet run" |
| [2026-09-12](2026-09-12-corpus-defences.md)                                                                      | Twelve clean diffs of the shapes a security tool is made of, and the ground truth behind them. A stub like the row above: the reviewer and the pair have not been run on them yet, so both commands say "not yet run"                    |
